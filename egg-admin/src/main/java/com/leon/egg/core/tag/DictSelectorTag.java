...
import com.leon.egg.modular.system.service.IDictService;
...
public class DictSelectorTag extends Tag {
	...
	public void render() {
		...
		if (ToolUtil.isEmpty(attrs.get("code"))) {
			throw new EggException(BizExceptionEnum.ERROR_CODE_EMPTY);
		}
		...
		int searchnum = ToolUtil.isNum(attrs.get("searchnum")) ? Integer.parseInt(attrs.get("searchnum").toString())
		        : 10;
		// 根据code查询字典数据
		...
		if ("radio".equals(type)) {
			...
			if (list.size() >= searchnum) {
				html.append(
				        "class=\"form-control chosen-select\" style=\"width:" + width + "px\"  tabindex=\"1\" \r\n");
			} else {
